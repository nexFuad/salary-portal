...
export async function createSalaryAdvance(c: Context<AppEnv>) {
  ...
  if (!valid(body))
    return c.json(
      {
        message:
          "Amount, reason, and a repayment period from 1 to 36 months are required",
      },
      400,
    );
...
export async function updateSalaryAdvance(c: Context<AppEnv>) {
  const body: unknown = await c.req.json().catch(() => null);
  if (!valid(body)) {
    return c.json(
      { message: "Please provide a valid salary advance request" },
      400,
    );
  }
  ...
  if (!result.count) {
    return c.json(
      { message: "Only your pending salary advance request can be updated" },
      404,
    );
  }
...
export async function cancelSalaryAdvance(c: Context<AppEnv>) {
  const result = await prisma.salaryAdvanceRequest.updateMany({
    where: {
      id: c.req.param("id")!,
      userId: c.get("authUser").sub,
  ...
  if (!result.count)
    return c.json(
      { message: "Only your pending salary advance request can be cancelled" },
      404,
    );
  return c.json({ message: "Salary advance request cancelled" });
}

export async function deleteSalaryAdvance(c: Context<AppEnv>) {
