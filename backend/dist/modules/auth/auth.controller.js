...
import { authenticateUser, createRefreshSession, findPublicUserById, revokeRefreshSession, rotateRefreshSession, toPublicUser, } from "./auth.service.js";
import { clearAuthCookies, refreshCookieName, setAuthCookies, } from "../../lib/auth.js";
function isLoginInput(value) {
    if (!value || typeof value !== "object")
        return false;
    const input = value;
    return (typeof input.employeeId === "string" &&
...
export async function updateProfile(c) {
    const body = (await c.req.json().catch(() => null));
    if (!body)
        return c.json({ message: "Profile details are required" }, 400);
    const name = typeof body.name === "string" ? body.name.trim() : undefined;
    const phone = typeof body.phone === "string" ? body.phone.trim() : undefined;
    const profilePic = typeof body.profilePic === "string" ? body.profilePic.trim() : undefined;
    ...
    const user = await import("../../lib/prisma.js").then(({ prisma }) => prisma.user.update({
