...
export async function getOfficerDashboard(company) {
    const now = new Date();
    const thisMonth = monthStart(now);
    const sixMonthStart = new Date(thisMonth);
    sixMonthStart.setUTCMonth(sixMonthStart.getUTCMonth() - 5);
    const [totalEmployees, activeEmployees, payrollRecords, pendingLeave, pendingAdvances, pendingLoans, recentLeaves, recentAdvances, recentLoans, recentPayroll,] = await Promise.all([
    ...
    const currentMonthPayroll = payrollRecords.filter((record) => monthKey(record.payRunMonth) === monthKey(thisMonth));
    const payrollStatus = currentMonthPayroll.reduce((summary, record) => {
